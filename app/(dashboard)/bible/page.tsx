...
interface FeedbackRow {
  name: string; email: string; phone: string; timestamp: string; rating: string;
  raw: Record<string, string>;
}

interface ChatRow { timestamp: string; type: string; message: string; sender: string; }
...
function parseAttendance(rows: Record<string, string>[]): AttendeeRow[] {
  ...
  rows.filter((r) => r[nameCol]?.trim()).forEach((r) => {
...
function parseFeedback(rows: Record<string, string>[]): FeedbackRow[] {
  ...
  return rows.filter((r) => r[nameCol]?.trim()).map((r) => ({
    name: (r[nameCol] || "").trim(),
    email: (r[emailCol] || "").trim().toLowerCase(),
    phone: (r[phoneCol] || "").trim(),
...
function parseChat(rows: Record<string, string>[]): ChatRow[] {
  return rows.map((r) => {
    ...
    return {
      timestamp: vals[0] || "", type: vals[1] || "", message: raw,
...
export default function BiblePage() {
  ...
  const [certDefs, setCertDefs] = useState<{id: string; label: string}[]>([]);
  ...
  // Load persisted attendance data whenever event changes
  useEffect(() => {
    if (!selectedEvent) { setAttendees([]); return; }
    setLoadingSaved(true);
    fetch(`/api/attendance?event_id=${selectedEvent}`)
      .then((r) => r.json())
      .then((data) => {
        if (Array.isArray(data) && data.length > 0) {
          setAttendees(data.map((d: {
            full_name: string; email: string; duration_seconds: number;
            camera_on: boolean; raw_data?: Record<string, string>;
          }) => ({
            name: d.full_name,
            email: d.email || "",
            phone: "",
            duration_seconds: d.duration_seconds,
            duration_minutes: Math.round(d.duration_seconds / 60),
            interaction: d.camera_on ? "interactive" : "streaming",
            platform: "", device: "",
            raw: d.raw_data || {},
          })));
          setAttendanceFile("(loaded from saved data)");
        }
  ...
  const parseFile = useCallback((file: File, onDone: (rows: Record<string, string>[]) => void, setName: (n: string) => void) => {
    setName(file.name);
    Papa.parse<Record<string, string>>(file, { header: true, skipEmptyLines: true, complete: (r) => onDone(r.data) });
  }, []);

  // Save attendance to Supabase whenever a new CSV is parsed (requires event selected)
  const handleAttendanceUpload = (file: File) => {
    if (!selectedEvent) {
      setSaveStatus("⚠️ Select an event first before uploading attendance");
      return;
    }
    parseFile(file, async (rows) => {
      const parsed = parseAttendance(rows);
      setAttendees(parsed);
      ...
      const res = await fetch("/api/attendance", {
        ...
        body: JSON.stringify({
          ...
          records: parsed.map((a) => ({
            full_name: a.name,
            email: a.email,
            duration_seconds: a.duration_seconds,
            camera_on: a.interaction === "interactive",
      ...
      setSaveStatus(data.success ? `✅ Saved ${data.saved} records to database` : `⚠️ ${data.error}`);
    }, setAttendanceFile);
  };
  ...
  const loadCertData = async () => {
    ...
    if (Array.isArray(certRes)) setCertDefs(certRes.map((c: {id: string; label: string}) => ({ id: c.id, label: c.label })));
  };
  ...
  const sendCertificates = async () => {
    if (!selectedEvent || !selectedTemplate || !selectedCert || !filteredAttendees.length) return;
    ...
    for (let i = 0; i < filteredAttendees.length; i += BATCH) {
      ...
      fd.append("certificate_template", selectedCert);
      fd.append("registrant_ids", JSON.stringify([]));
      fd.append("attendees_json", JSON.stringify(batch.map((a) => ({ full_name: a.name, email: a.email, phone: a.phone }))));
  ...
  return (
    <div className="max-w-6xl">
      <div className="mb-8">
        ...
        <p className="text-slate-400 text-sm mt-1">Filter, analyse and act on post-event data — saved automatically per event</p>
      </div>

      {/* Event selector — required first step now */}
      <div className="bg-slate-900 rounded-xl border border-slate-800 p-5 mb-6">
        ...
        <select value={selectedEvent} onChange={(e) => setSelectedEvent(e.target.value)} className={inputCls}>
          ...
          {events.map((ev) => (
            <option key={ev.id} value={ev.id}>{ev.name} · {new Date(ev.event_date).toLocaleDateString("en-IN")}</option>
          ))}
        </select>
        {loadingSaved && <p className="text-xs text-slate-500 mt-2">Loading saved data…</p>}
        {!selectedEvent && (
          <p className="text-xs text-yellow-400 mt-2">⚠️ Select an event to view or upload attendance data — this ties your data to that event permanently.</p>
        )}
      </div>
      ...
      {selectedEvent && (
        <>
          ...
          {saveStatus && (
            <div className="mb-4 text-xs text-slate-400 bg-slate-900 border border-slate-800 rounded-lg px-4 py-2">{saveStatus}</div>
          )}
          ...
          {tab === "attendance" && (
            <div>
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                      <div>
                        ...
                        <input type="number" value={minDuration} min={0}
                          onChange={(e) => setMinDuration(parseInt(e.target.value) || 0)} className={inputCls} />
                      ...
                      <div>
                        ...
                        <div className="flex flex-col gap-2 mt-1">
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" checked={requireFeedback} onChange={(e) => setRequireFeedback(e.target.checked)} className="rounded border-slate-600" />
                            <span className="text-sm text-slate-300">Must have filled feedback</span>
                          </label>
                          {requireFeedback && feedback.length === 0 && <p className="text-xs text-yellow-400">⚠️ Upload feedback CSV first</p>}
                        </div>
                      </div>
                      <div>
                        <label className="block text-xs text-slate-400 mb-1">Search</label>
                        <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Name or email..." className={inputCls} />
                      </div>
                    </div>

                    <div className="flex items-center gap-3 flex-wrap">
                      <div className="flex gap-2 flex-wrap">
                        {minDuration > 0 && <span className="text-xs px-2 py-1 bg-brand-500/10 text-brand-400 border border-brand-500/20 rounded-full">⏱ {minDuration}+ minutes</span>}
                        {requireFeedback && <span className="text-xs px-2 py-1 bg-purple-500/10 text-purple-400 border border-purple-500/20 rounded-full">📝 Filled feedback</span>}
                        {searchQuery && <span className="text-xs px-2 py-1 bg-slate-700 text-slate-300 rounded-full">🔍 &quot;{searchQuery}&quot;</span>}
                        {(minDuration > 0 || requireFeedback || searchQuery) && (
                          <button onClick={() => { setMinDuration(0); setRequireFeedback(false); setSearchQuery(""); }} className="text-xs px-2 py-1 text-red-400 hover:text-red-300">× Clear all</button>
                        )}
                      </div>
                      <div className="ml-auto flex gap-2">
                        <button onClick={() => downloadCSV(
                          filteredAttendees.map((a) => ({ Name: a.name, Email: a.email, Phone: a.phone, "Duration (min)": a.duration_minutes, "Filled Feedback": hasFeedback(a) ? "Yes" : "No" })),
                          `filtered-${filteredAttendees.length}-attendees.csv`
                        )} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg text-slate-300 text-xs font-semibold">↓ Download ({filteredAttendees.length})</button>
                        <button onClick={handleShowCertPanel} disabled={filteredAttendees.length === 0}
                          className="px-4 py-2 bg-brand-600 hover:bg-brand-500 disabled:opacity-40 rounded-lg text-white text-xs font-semibold">🎓 Send Certificates ({filteredAttendees.length})</button>
                      </div>
                    </div>
                  </div>
                  ...
                  {showCertPanel && (
                    <div className="bg-slate-900 rounded-xl border border-brand-500/30 p-5 mb-4">
                      <h3 className="text-sm font-semibold text-white mb-4">Send Certificates to {filteredAttendees.length} filtered attendees</h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                          <label className="block text-xs text-slate-400 mb-1">Certificate Design *</label>
                          <select value={selectedCert} onChange={(e) => setSelectedCert(e.target.value)} className={inputCls}>
                      ...
                      <div className="mb-4">
                        ...
                        {handoutFile ? (
                          ...
                          <label className="block cursor-pointer bg-slate-800 border border-dashed border-slate-700 hover:border-slate-600 rounded-lg px-3 py-2 text-sm text-slate-500 text-center">
                            Click to upload handout PDF
                            <input type="file" accept=".pdf" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) setHandoutFile(f); }} />
                          </label>
                        )}
                      </div>
                      ...
                      {sending && (
                        <div className="mb-4">
                          <div className="flex justify-between text-xs text-slate-400 mb-1"><span>Sending {sendProgress.current} / {sendProgress.total}</span><span>{pct}%</span></div>
                          <div className="w-full bg-slate-800 rounded-full h-2 mb-1"><div className="bg-brand-500 h-2 rounded-full transition-all" style={{ width: `${pct}%` }} /></div>
                      ...
                      <div className="flex gap-3">
                        <button onClick={sendCertificates} disabled={!selectedTemplate || !selectedCert || sending}
                        ...
                        <button onClick={() => setShowCertPanel(false)} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-slate-300 text-sm">Cancel</button>
                      </div>
                    </div>
                  )}
                </>
              ...
              {attendees.length === 0 ? (
                <EmptyState label="Upload an attendance CSV for this event to get started" />
              ) : (
                        ...
                        {filteredAttendees.map((a, i) => (
                          <tr key={i} className="border-t border-slate-800/50 hover:bg-slate-800/20">
                            ...
                            <td className="px-4 py-2.5">
                              <span className={`text-xs font-medium ${a.duration_minutes >= 60 ? "text-green-400" : a.duration_minutes >= 30 ? "text-yellow-400" : "text-red-400"}`}>{a.duration_minutes} min</span>
                            </td>
                            <td className="px-4 py-2.5">
                              {feedback.length > 0 ? (
                                <span className={`text-xs px-2 py-0.5 rounded font-medium ${hasFeedback(a) ? "bg-purple-500/10 text-purple-400" : "bg-slate-800 text-slate-600"}`}>{hasFeedback(a) ? "✅ Yes" : "No"}</span>
            ...
            <div>
              {feedback.length > 0 && (
                <div className="bg-slate-900 rounded-xl border border-slate-800 p-4 mb-4 flex gap-3">
                  <input value={feedbackSearch} onChange={(e) => setFeedbackSearch(e.target.value)} placeholder="Search name or email..." className="flex-1 bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-brand-500" />
                  <button onClick={() => downloadCSV(filteredFeedback.map((f) => f.raw), "feedback-filtered.csv")} className="px-4 py-2 bg-brand-600 hover:bg-brand-500 rounded-lg text-white text-xs font-semibold whitespace-nowrap">↓ Download ({filteredFeedback.length})</button>
                </div>
              )}
                ...
                <div className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden">
                  <div className="overflow-x-auto max-h-[500px] overflow-y-auto">
                    <table className="w-full text-sm">
                      <thead className="sticky top-0 bg-slate-800/90">
                        <tr className="text-xs text-slate-500 uppercase">
                          <th className="text-left px-4 py-3">Name</th><th className="text-left px-4 py-3">Email</th><th className="text-left px-4 py-3">Phone</th><th className="text-left px-4 py-3">Rating</th><th className="text-left px-4 py-3">Submitted</th>
                        </tr>
                      </thead>
                      <tbody>
                        {filteredFeedback.map((f, i) => (
                          <tr key={i} className="border-t border-slate-800/50 hover:bg-slate-800/20">
                            ...
                            <td className="px-4 py-2.5"><span className="text-xs px-2 py-0.5 rounded font-medium bg-slate-800 text-slate-400">{f.rating || "—"}</span></td>
          ...
          {tab === "chat" && (
            <div>
              {chat.length > 0 && (
                <div className="bg-slate-900 rounded-xl border border-slate-800 p-4 mb-4 flex gap-3">
                  <input value={chatSearch} onChange={(e) => setChatSearch(e.target.value)} placeholder="Search messages or sender..." className="flex-1 bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-brand-500" />
                  <button onClick={() => downloadCSV(filteredChat.map((c) => ({ Timestamp: c.timestamp, Type: c.type, Sender: c.sender, Message: c.message })), "chat-filtered.csv")} className="px-4 py-2 bg-brand-600 hover:bg-brand-500 rounded-lg text-white text-xs font-semibold whitespace-nowrap">↓ Download ({filteredChat.length})</button>
                </div>
              )}
...
function UploadCard({ label, file, onFile, count, icon }: { label: string; file: string; onFile: (f: File) => void; count: number; icon: string; }) {
  return (
    <label className={`block bg-slate-900 rounded-xl border-2 border-dashed p-5 cursor-pointer transition-colors ${file ? "border-brand-500/50 bg-brand-500/5" : "border-slate-700 hover:border-slate-600"}`}>
      <div className="text-2xl mb-2">{icon}</div>
      <p className="text-sm font-medium text-white">{label}</p>
      {file ? <p className="text-xs text-green-400 mt-1">✓ {file} · {count} rows</p> : <p className="text-xs text-slate-500 mt-1">Click to upload CSV</p>}
...
function StatBox({ label, value, color }: { label: string; value: string | number; color: string }) {
  const colors: Record<string, string> = { blue: "text-blue-400", green: "text-green-400", purple: "text-purple-400", yellow: "text-yellow-400" };
  return (
...
function EmptyState({ label }: { label: string }) {
  return <div className="bg-slate-900 rounded-xl border border-slate-800 px-5 py-12 text-center text-slate-500 text-sm">{label}</div>;
}